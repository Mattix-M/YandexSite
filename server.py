...
def index():
    ...
    return render_template('index.html', title='Говно-сайт', username=session['username'],  cur_user_id=session['user_id'],
...
def user_edit():
    ...
    return render_template('edit.html', title='Редактирование', form=form, my_page=False, cur_user_id=session['user_id'], username=session['username'])  
...
def add_news():
    ...
    if form.validate_on_submit():
        ...
        return redirect("/index")
    return render_template('add_news.html', title='Добавление новости',  cur_user_id=session['user_id'],
...
if __name__ == '__main__':
    app.run(port=6969, host='127.0.0.1')